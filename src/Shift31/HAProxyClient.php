...
use Zend\Http\Client, Zend\Http\Request;
...
class HAProxyClient
{
    ...
    public function setStatsUrl($serverFqdn)
    {
        $this->_serverFqdn = $serverFqdn;
        $this->_statsUrl
            = "http://{$this->_username}:{$this->_password}@$serverFqdn:{$this->_port}{$this->_baseUrl};csv";
    }
    ...
    public function checkServiceStatusInCluster(array $loadBalancers, $proxyName, $serviceName, $statusToCheck = 'UP')
    {
        ...
        while ($statusVerified === false && $lbTries < $this->_checkMaxRetries) {
            ...
            if (count($clusterStats) <= 0) {
                $this->_log(
                    'alert', "Unable to retrieve stats from any of $serviceName's load balancers. Please contact a Systems Administrator."
                );
            ...
            foreach ($clusterStats as $loadBalancer => $stats) {
                ...
                if (isset($stats[$proxyName][$serviceName])) {
                    ...
                    $this->_log(
                        'debug', "$loadBalancer: Status for service '$serviceName' in proxy '$proxyName' is '$proxyServiceStatus'"
                    );
                    ...
                    if ($statusToCheck == 'UP') {
                        ...
                        && ($stats[$proxyName][$serviceName]['scur'] == '0') ? true : false;
                    }

                } else {
                    $this->_log(
                        'alert', "Unable to find service named '$serviceName' in proxy named '$proxyName' on $loadBalancer. Please contact a Systems Administrator."
                    );
        ...
        if ($statusVerified === true) {
            $this->_log(
                'info', "HAProxy detected service '$serviceName' of proxy '$proxyName' is '$statusToCheck'.  Retried $lbTries times ("
                . $lbTries * 2 . ' seconds).'
            );
        } elseif ($noServiceInProxy === false) {
            $this->_log(
                'warn', "HAProxy did not detect service '$serviceName' of proxy '$proxyName' as '$statusToCheck'.  Retried $lbTries times ("
                . $lbTries * 2 . ' seconds).'
            );
        }
    }
